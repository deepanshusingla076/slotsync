  ...
  try {
    ...
    if (!name || !email || !event_type_id || !date || !time) {
      return res
        .status(400)
        .json({
          error: "name, email, event_type_id, date, and time are required",
        });
    }
    ...
    if (isNaN(dateObj.getTime())) {
      return res
        .status(400)
        .json({ error: "Invalid date. Use format YYYY-MM-DD" });
    }
    ...
    if (!/^\d{2}:\d{2}$/.test(time)) {
      return res
        .status(400)
        .json({ error: "Invalid time. Use format HH:MM (e.g. 10:00)" });
    }
    ...
    if (!eventType.is_active) {
      return res
        .status(400)
        .json({ error: "This event type is no longer active" });
    }

    // Step 3: Build timestamp strings
    // PostgreSQL timestamp format: "YYYY-MM-DD HH:MM:SS"
    const startDateTime = new Date(`${date}T${time}:00`);
    const endDateTime = new Date(
      startDateTime.getTime() + eventType.duration_minutes * 60 * 1000,
    );
    ...
    if (!availability || !availability.is_available) {
      return res
        .status(400)
        .json({ error: "The host is not available on this day" });
    }
