...
export default function TimeSlotPicker({ slots, selectedTime, onSelect, loading }) {
  ...
  if (!slots || slots.length === 0) {
    return (
      <div className="flex flex-col items-center py-10 text-center bg-slate-50/50 border border-dashed border-slate-200 rounded-xl">
        <div className="w-11 h-11 bg-white rounded-xl shadow-sm border border-slate-100 flex items-center justify-center mb-3">
  ...
  return (
    <div className="flex flex-col gap-2 overflow-y-auto max-h-[56dvh] lg:max-h-[420px] pr-1 -mr-1">
      {slots.map(slot => (
        <button
          key={slot.time}
          ...
          className={`
            w-full py-2.5 px-3 rounded-xl text-center font-bold text-sm transition-all duration-200 border
